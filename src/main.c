...
void maintask(void *ignore)
{
   ...
   char character[2] = {};
    ...
    uint8_t testdata[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZABCDE";
    status = encryptwrite(testdata, sizeof(testdata));

    if (status != ATCA_SUCCESS) {
	ESP_LOGE(TAG_ECC608, "encryptwrite() failed with ret=0x%08d/r/n", status);
    }

    uint8_t verifydata[240] = {};
    status = decryptread(verifydata, sizeof(verifydata));

    if (status != ATCA_SUCCESS) {
	ESP_LOGE(TAG_ECC608, "decryptread() failed with ret=0x%08d/r/n", status);
    }

    if (memcmp(testdata, verifydata, 240) != 0)
    {
